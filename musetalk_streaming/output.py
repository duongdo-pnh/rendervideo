...
class RTMPOutput(StreamOutput):
    """Persistent FFmpeg transport for RTMP or local MPEG-TS/UDP."""
    ...
    def __init__(
        ...
        if self._scheme not in {"rtmp", "rtmps", "udp"}:
            raise ValueError("push_url must use rtmp://, rtmps:// or udp://")
    ...
    def _spawn(self) -> None:
        ...
        if self._scheme == "udp":
            output_args = ["-flush_packets", "1", "-f", "mpegts", self._push_url]
            # UDP is only the local OBS handoff. NVENC uses the 3090's dedicated
            # encoder, preserving the same frame size/FPS/bitrate while freeing
            # CPU for MuseTalk frame blending.
            video_encoder = ["-c:v", "h264_nvenc", "-preset", "p5", "-tune", "ll"]
        else:
            output_args = ["-f", "flv", self._push_url]
            video_encoder = ["-c:v", "libx264", "-preset", "veryfast", "-tune", "zerolatency"]
        command = [
            ...
            *(["-vf", f"fps={self._output_fps}"] if self._output_fps and self._output_fps != fps else []),
            ...
            "-b:a", self._audio_bitrate, *output_args,
        ]
        try:
